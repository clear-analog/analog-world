...
void ADS1299_WREG(uint8_t regAdd, uint8_t *values, uint8_t numRegs)
{
    ...
    // is this separate from handshake? 
      // take in parameters to change e.g. sampling rate
...
// Check if we're using ESP32 or AVR
    ...
    // --- Get timestamp from BrainFlow ---
    unsigned long start_wait = millis();
    // while (!_timestamp_initialized && (millis() - start_wait) < 15000)
    // { // 30 second timeout
    //     if (Serial.available())
    //     {
    //         // Wait for command to start streaming data
    //         String input = Serial.readStringUntil('\n');
    //         input.trim(); // Remove whitespace

    //         // Command or input is received
    //         if (input.length() > 0)
    //         {
    //             // Ignore BrainFlow's "b" command during handshake
    //             if (input == "b") continue;

    //             if (input == "config file") continue; // parameter string
                  
    //               // buffer contains
    //               // start marker, message type, timestamp, data, checksum, end marker
    //               // for data

    //               // buffer contains
    //               // start marker, message type, parameters (regval pair), checksum, end marker
    //               // array of Deez
    //               // for parameters

    //             // Check if input is valid UNIX time
    //             uint32_t parsed_time = input.toInt();
    //             if (parsed_time > 1700000000UL)
    //             { // 1.7B is November 2023
    //                 _unix_timestamp_reference = parsed_time;
    //                 _millis_reference = millis();
    //                 _timestamp_initialized = true;
    //                 DEBUG_PRINT("Timestamp initialized: ");
    //                 DEBUG_PRINTLN(_unix_timestamp_reference);
    //             }
    //             else
    //             {
    //                 DEBUG_PRINTLN("Invalid timestamp. Please send a valid Unix timestamp (e.g., 1718918556)");
    //             }
    //         }
    //     }
    //     delay(100);
